( async( ) => {
  ...
  document.getElementById("messageForm").addEventListener("submit", async function (e) {
    ...
    try {
      const res = await fetch(`${json.API_BASE}/message`, {
        method: "POST",
        headers: {
  ...
  async function chargerMessages() {
    ...
    try {
      const res = await fetch(`${json.API_BASE}/messages?nb_messages=10`);
  ...
  async function likerMessage(messageId) {
    try {
      const res = await fetch(`${json.API_BASE}/message/like`, {
        method: "POST",
  ...
  async function chargerCommentaires(messageId) {
    ...
    try {
      const res = await fetch(`${json.API_BASE}/comments?message_id=${messageId}`);
  ...
  async function ajouterCommentaire(messageId, username, content) {
    try {
      const res = await fetch(`${json.API_BASE}/comment`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
